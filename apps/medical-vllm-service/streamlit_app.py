...
st.title("Medical-RAG 医疗影像混合模态 RAG")
st.caption("FastAPI + Streamlit 交互页面")
...
with right:
    question = st.text_area(
        "输入医学问题",
        value="请结合影像、检索到的指南证据和参考影像，分析主要异常并给出下一步建议。",
        height=180,
    )
...
if run:
    if image_file is None:
        ...
        if payload:
            ...
            hyde_text = payload.get("hyde_text", "")
            if hyde_text:
                with st.expander("查看 HyDE 伪文档"):
                    st.write(hyde_text)
