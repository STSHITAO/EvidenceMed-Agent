...
def _format_evidence_md(answer) -> str:
    ...
    for i, item in enumerate(answer.evidence, start=1):
        if item.modality == "image":
            rows.append(
                f"{i}. **来源**: `{item.source}` | **type**: `image` | **score**: `{item.score:.4f}`\n"
                f"   描述: {item.content[:450]}{'...' if len(item.content) > 450 else ''}"
            )
        else:
            rows.append(
                f"{i}. **来源**: `{item.source}` | **chunk**: `{item.chunk_index}` | **type**: `{item.modality}` | **score**: `{item.score:.4f}`\n"
...
def build_demo(settings: Settings) -> gr.Blocks:
    ...
    with gr.Blocks(title="Medical-RAG") as demo:
        gr.Markdown(
            ...
            上传医学影像并提问，系统会先进行多模态检索，再结合文本证据与参考影像完成联合推理。
            """
        )
        ...
        with gr.Row():
            ...
            question = gr.Textbox(
                lines=5,
                label="输入问题",
                placeholder="例如：请结合影像与检索证据，分析可疑征象并给出下一步建议。",
            )
