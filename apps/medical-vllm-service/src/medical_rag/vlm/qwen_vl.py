...
from medical_rag.prompts import SYSTEM_PROMPT, build_hyde_prompt, build_user_prompt
...
class QwenVLMReasoner:
    ...
    def _build_messages(self, image_paths: List[str], user_text: str) -> list:
    ...
    def _chat_vllm(self, messages: list, max_tokens: int, temperature: float) -> str:
        content_parts = messages[1]["content"]
        payload_content = []
        for item in content_parts:
            if item["type"] == "text":
                payload_content.append({"type": "text", "text": item["text"]})
            else:
                payload_content.append(
                    {
                        "type": "image_url",
                        "image_url": {"url": self._image_to_data_url(item["image"])},
                    }
                )
    ...
    def _generate_transformers(self, messages: list, max_tokens: int, temperature: float) -> str:
    ...
    def _run_chat(self, messages: list, max_tokens: int, temperature: float) -> str:
        if self.backend == "vllm_openai":
            return self._chat_vllm(messages=messages, max_tokens=max_tokens, temperature=temperature)
        return self._generate_transformers(messages=messages, max_tokens=max_tokens, temperature=temperature)
    ...
    def generate(
        ...
        evidence_image_paths: List[str] | None = None,
    ) -> str:
        evidence_image_paths = evidence_image_paths or []
        user_prompt = build_user_prompt(
            question=question,
            evidence_blocks=evidence_blocks,
            evidence_image_count=len(evidence_image_paths),
        )
        image_paths = [image_path] + evidence_image_paths
        messages = self._build_messages(image_paths=image_paths, user_text=user_prompt)
    ...
    def generate_hypothetical_document(
        ...
        hyde_prompt = f"{prompt}\n\n{build_hyde_prompt(question)}"
        image_paths = [image_path] if image_path else []
        messages = self._build_messages(image_paths=image_paths, user_text=hyde_prompt)
        return self._run_chat(
            messages=messages,
