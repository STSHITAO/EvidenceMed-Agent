...
class MilvusVectorStore:
    ...
    def _build_schema(self, vector_dim: int) -> CollectionSchema:
        fields = [
            ...
            FieldSchema(name="asset_path", dtype=DataType.VARCHAR, max_length=2048),
            FieldSchema(name="metadata_json", dtype=DataType.VARCHAR, max_length=8192),
            FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, dim=vector_dim),
        ]
        return CollectionSchema(fields=fields, description="Medical multimodal knowledge items")
    ...
    def search(self, query_vector: np.ndarray, top_k: int) -> List[RetrievedChunk]:
        ...
        results = self.collection.search(
            ...
            param=search_params,
            limit=top_k,
            output_fields=["doc_id", "source", "chunk_index", "content", "modality", "asset_path", "metadata_json"],
        )
        ...
        for hit in results[0]:
            ...
            merged.append(
                RetrievedChunk(
                    ...
                    modality=hit.entity.get("modality") or "text",
                    asset_path=hit.entity.get("asset_path") or "",
                    metadata=metadata,
                )
            )
        return merged
